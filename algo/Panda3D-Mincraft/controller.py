...
class Controller():
    ...
    def __init__(self):
        ...
        # создание конвейера столкновений
        ...
        # уведомляем конвейер о новом «объекте ОТ»
    ...
    #ToDo Хотим добавить столкновение с финишным блоком -
    # для этого хотим добавить еще один один конвеер
    # и поместить в него финишный блок
    # либо найти в Редакторе карт как в очередь,
    # или конвеер попадают блоки и как среди них найти финишный
    ...
    def collisionTest(self):
        ...
        if self.collisQueue.getNumEntries() > 0:
            # Тестируем доступ к свойствам объектов при касании - для поиска нужного финального куба
            #for entry in self.collisQueue.getEntries():
            #    with open("log_entry.txt", "a", encoding="utf-8") as file:
            #        file.write(f"{entry}\n")
            #entries = self.collisQueue.getEntries()
            #entry = self.collisQueue.getEntry(0)
            #with open("queue.log", "a", encoding="utf-8") as file:
            #    file.write("\n{}\n into: \n {}".format(entry.getIntoNodePath(), entry.getInto()))
            return True
        else:
            return False
